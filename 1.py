...
class TripletLoss(torch.nn.modules.loss._Loss):
    ...
    def forward(self, batch, encoder, train, neg_samples, save_memory=False):
        # print("batch_size:", batch.size(), "type:", type(batch))
        # print("train_size:", train.size(), "type:", type(train))
        ...
        #随机采样训练样本的大小，从0-train——size选取self.nb_random_samples*batch_size
        ...
        #选择时间序列  参考样本的起始位置 Start of anchors
        ...
        for i in range(self.nb_random_samples):
            ...
            # temp = torch.cat([neg_sample[j: j + 1, :,beginning_samples_neg[i, j]:beginning_samples_neg[i, j] + length_pos_neg] for j in range(batch_size)])
            ...
            # print("negative_representation:  ", negative_representation.shape)
